...
def radpress(pressure, temperature, mu, p0, R0, g):
  ...
  # Return back to desending order for radius calculation -- large to small
  press = pressure   [::-1]
  temp  = temperature[::-1]
  mu    = mu         [::-1]
  ...
  # in atmfile file -- connected to pressures small to large:
  rad = rad[::-1]
...
def mean_molar_mass(abun_file, atmfile):
    ...
    # Read the atmospheric file, pressure array -- small to large:
...
def makeRadius(out_spec, atmfile, abun_file, tepfile, p0):
  ...
  # Read the final atmospheric file, pressure array -- small to large
